public class LinkedList {
    ...
    public boolean isInIncreasingOrder() {
        ...
        return isInIncreasingOrder(this.head);
    }

    private boolean isInIncreasingOrder(Item item) {
        // Base case: List with length of 0 (Terminate)
        if (item == null)
            return true;

        // Base case: First item (head - skip to next item)
        if (item.previous == null)
            return isInIncreasingOrder(item.next);

        // Base case: Reached the end of the list (Terminate)
        if (item.next == null)
            return item.data >= item.previous.data;

        // Base case: Not in increasing order (Terminate)
        if (item.data < item.previous.data) return false;

        // Recursive case: Check the next item
        return isInIncreasingOrder(item.next);
    }
}
